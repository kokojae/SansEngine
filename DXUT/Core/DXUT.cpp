...
// 선택적으로 명령 행을 구문 분석하고 기본 단축키가 처리되는지 설정
//
//       Possible command line parameters are:
//       가능한 명령 줄 매개 변수는 다음과 같습니다.
...
//									앱이 HAL을 사용하도록 강제합니다 (HAL이 없으면 실패)
                         ...
                         bool bThreadSafeDXUT )
{
	...
	// 항상 필요한 것은 아니지만 앱에서 GDI 대화 상자를 만들 수 있음
    InitCommonControls();

    // Save the current sticky/toggle/filter key settings so DXUT can restore them later
	// DXUT가 나중에 복원 할 수 있도록 현재 고정 / 토글 / 필터 키 설정을 저장합니다.
...
// 지정된 창 제목, 아이콘, 메뉴 및 시작 위치로 창을 만듭니다.
// DXUTInit ()가 아직 호출되지 않은 경우 기본 매개 변수를 사용하여 호출합니다.
// 이것을 호출하는 대신 DXUTSetWindow ()를 호출하여 기존 창을 사용할 수 있습니다.
//--------------------------------------------------------------------------------------
...
// 유휴 상태 일 때 앱의 메시지 루프 및 렌더링을 처리합니다. DXUTCreateDevice () 또는 DXUTSetD3D * Device ()가
// 아직 호출되지 않은 경우 기본 매개 변수와 함께 DXUTCreateWindow ()를 호출합니다.
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTMainLoop( HACCEL hAccel )
{
	...
	// 장치 콜백 내부에서 전화를 걸거나 재 입력 할 수 없습니다
	...
	// DXUTCreateDevice () 또는 DXUTSetD3D * Device ()가 아직 호출되지 않은 경우
	// 기본 매개 변수를 사용하여 DXUTCreateDevice ()를 호출하십시오.
    if( !GetDXUTState().GetDeviceCreated() )
    {
        if( GetDXUTState().GetDeviceCreateCalled() )
        {
            if( ( GetDXUTState().GetExitCode() == 0 ) || ( GetDXUTState().GetExitCode() == 10 ) )
                GetDXUTState().SetExitCode( 1 );
            return E_FAIL; // DXUTCreateDevice() must first succeed for this function to succeed, 이 함수가 성공하려면 DXUTCreateDevice ()가 먼저 성공해야합니다
        }
	...
	// 이 함수를 진행하려면 DXUTCreateDevice () 또는 DXUTCreateDeviceFromSettings () 또는 DXUTSetD3D * Device ()가 호출되어 성공해야합니다.
	...
	// 이제 Windows 메시지를 받고 처리 할 준비가되었습니다.
    bool bGotMsg;
    MSG msg;
    msg.message = WM_NULL;
    ...
    while( WM_QUIT != msg.message )
    {
		...
		// 유휴 시간을 사용하여 장면을 렌더링 할 수 있도록 PeekMessage ()를 사용하십시오.
        ...
        else
        {
			...
			// 대기 시간 동안 프레임을 렌더링합니다 (대기중인 메시지 없음)
